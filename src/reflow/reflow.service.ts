...
export class ReflowService {
  ...
  reflow(input: ReflowServiceInput): ReflowResult {
    ...
    const workflowGraph = this.workflowBuilderService.buildWorkflow(
      sortedMaintenanceWorkOrders.concat(sortedOtherWorkOrders),
    );
    const sortedWorkOrderNumbers =
      this.workflowBuilderService.getSortedWorkOrderNumbers(workflowGraph);
    ...
    for (const workOrderNumber of sortedWorkOrderNumbers) {
      const workOrder = workflowGraph.getNodeAttribute(
        workOrderNumber,
        'workOrder',
      );
      if (workOrder.isMaintenance) {
        // Keep the maintenance work order in the same position
        completionTimesOfCompletedWorkOrders.set(
          workOrderNumber,
          DateTime.fromISO(workOrder.endDate),
        );
        ...
        bookedSlotsByCenter.set(workOrder.workCenterId, [
          ...(bookedSlotsByCenter.get(workOrder.workCenterId) || []),
          Interval.fromDateTimes(
            DateTime.fromISO(workOrder.startDate),
            DateTime.fromISO(workOrder.endDate),
          ),
        ]);
        continue;
      ...
      for (const parentId of workOrder.dependsOnWorkOrderIds) {
        if (parentId in completionTimesOfCompletedWorkOrders) {
          earliestStart = DateTime.max(
            earliestStart,
            completionTimesOfCompletedWorkOrders.get(parentId)!,
          );
      ...
      if (!ReflowService.isAvailable(workCenter, earliestStart)) {
        ...
        earliestStart = ReflowService.nextAvailableMoment(
          workCenter,
          earliestStart,
        );
        ...
        if (
          !earliestStart.equals(
            DateTime.fromISO(
              parentChange ? parentChange.newValue : workOrder.startDate,
            ),
          )
        ) {
      ...
      // get the booked slot
      const interval = ReflowService.firstFreeSlot(
        workCenter,
        bookedSlotsByCenter.get(workCenter.name) || [],
        earliestStart,
        duration,
      );
      ...
      bookedSlotsByCenter.set(workCenter.name, [
        ...(bookedSlotsByCenter.get(workCenter.name) || []),
        interval,
      ]);
    }
    ...
    return {
      updatedWorkOrders: Array.from(assignments.values()).map(
        ({ workOrder }) => workOrder,
      ),
  ...
  static isInMaintenanceWindow(
    ...
    for (const maintenanceWindow of workCenter.maintenanceWindows) {
      if (
        dateTime >= DateTime.fromISO(maintenanceWindow.startDate) &&
        dateTime < DateTime.fromISO(maintenanceWindow.endDate)
      ) {
        return true;
      }
    }
  ...
  static isAvailable(workCenter: WorkCenterData, dateTime: DateTime): boolean {
    return (
      ReflowService.isInShift(workCenter, dateTime) &&
      !ReflowService.isInMaintenanceWindow(workCenter, dateTime)
    );
  }

  /** Find the next time where the work center is available */
  static nextAvailableMoment(
    workCenter: WorkCenterData,
    dateTime: DateTime,
  ): DateTime {
  ...
  static addWorkingMinutes(
    ...
    while (remainingMinutes > 0) {
      if (!ReflowService.isAvailable(workCenter, currentTime)) {
        currentTime = ReflowService.nextAvailableMoment(
          workCenter,
          currentTime,
        );
        continue;
  ...
  static firstFreeSlot(
    ...
    while (attempts < maxAttempts) {
      const candidateEndTime = ReflowService.addWorkingMinutes(
        workCenter,
        candidateStartTime,
        durationMinutes,
      );
      ...
      for (const interval of sortedBookedSlots) {
        if (
          Interval.fromDateTimes(candidateStartTime, candidateEndTime).overlaps(
            interval,
          )
        ) {
          overlaps = true;
          candidateStartTime = ReflowService.nextAvailableMoment(
            workCenter,
            interval.end!,
          );
          break;
  ...
  static populateWorkCenters(
    workCenters: WorkCenterDocument[],
  ): Map<string, WorkCenterData> {
  ...
  static populateWorkOrdersByCenter(
    workOrders: WorkOrderDocument[],
  ): Map<string, WorkOrderData[]> {
    const workOrdersByCenter = new Map<string, WorkOrderData[]>();
    for (const workOrder of workOrders) {
      workOrdersByCenter.set(workOrder.data.workCenterId, [
  ...
  static generateWhatChanged(changes: ReflowChange[]): string[] {
    return changes.map(
      (change) =>
        `${change.workOrderNumber} changed the ${change.changeType} from ${change.oldValue} to ${change.newValue}`,
    );
  }
  ...
  static generateExplanation(changes: ReflowChange[]): string[] {
    return changes.map(
      (change) =>
        `${change.workOrderNumber} changed the ${change.changeType} from ${change.oldValue} to ${change.newValue} because ${change.reason}`,
    );
  }
}
