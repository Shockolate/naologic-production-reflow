...
function isWorkOrderDocumentDto(
  doc: ReflowDocumentDto,
): doc is WorkOrderDocumentDto {
  return doc.docType === 'workOrder';
}

function isWorkCenterDocumentDto(
  doc: ReflowDocumentDto,
): doc is WorkCenterDocumentDto {
  return doc.docType === 'workCenter';
}

function isManufacturingOrderDocumentDto(
  doc: ReflowDocumentDto,
): doc is ManufacturingOrderDocumentDto {
...
export class ReflowController {
  ...
  processReflow(@Body() body: ProcessReflowDto) {
    const input = {
      ...
      manufacturingOrders: body.documents.filter(
        isManufacturingOrderDocumentDto,
      ),
    };
