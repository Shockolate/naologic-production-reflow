...
export class WorkflowBuilderService {
  ...
  ): DirectedGraph<{ workOrder: WorkOrderData }> {
    ...
    for (const workOrder of workOrders) {
      ...
      for (const dependsOnWorkOrderId of workOrder.dependsOnWorkOrderIds) {
        if (
          ...
          throw new Error(
            `WorkOrder ${workOrder.workOrderNumber} will create a cycle with ${dependsOnWorkOrderId}.`,
          );
        }
  ...
  getSortedWorkOrderNumbers(
    graph: DirectedGraph<{ workOrder: WorkOrderData }>,
  ): string[] {
