...
describe(ReflowService.name, () => {
  ...
  describe('sortBookedSlots', () => {
    it('should sort the booked slots by start time', () => {
      ...
      expect(ReflowService.sortBookedSlots(bookedSlots)).toEqual(
        bookedSlots.sort((a, b) => a.start!.diff(b.start!).toMillis()),
      );
    });
    ...
    it('should sort the booked slots by end time', () => {
      ...
      expect(ReflowService.sortBookedSlots(bookedSlots)).toEqual(
        bookedSlots.sort((a, b) => a.end!.diff(b.end!).toMillis()),
      );
    });
  });
  ...
  describe('firstFreeSlot', () => {
    it('should find the slot after all existing bookings', () => {
      ...
      const result = ReflowService.firstFreeSlot(
        wc,
        bookedSlots,
        start,
        duration,
    ...
    it('should skip to the next week if no slot is available today', () => {
      ...
      expect(result.end!.toISO()).toBe(
        expectedStart.plus({ minutes: 60 }).toISO(),
      );
    });
  ...
  describe('reflow', () => {
    ...
    it('should throw if workOrder refers to unknown workCenter', () => {
      ...
      expect(() => service.reflow(input)).toThrow(
        /Work center NON-EXISTENT not found/,
      );
    });
    ...
    it('should handle overlapping work orders on the same work center', () => {
      ...
      expect(
        DateTime.fromISO(result.updatedWorkOrders[0].startDate).equals(
          DateTime.fromObject({
            year: 2026,
            month: 1,
            day: 5,
      ...
      expect(
        DateTime.fromISO(result.updatedWorkOrders[0].endDate).equals(
          DateTime.fromObject({
            year: 2026,
            month: 1,
            day: 5,
      ...
      expect(
        DateTime.fromISO(result.updatedWorkOrders[1].startDate).equals(
          DateTime.fromObject({
            year: 2026,
            month: 1,
            day: 5,
      ...
      expect(
        DateTime.fromISO(result.updatedWorkOrders[1].endDate).equals(
          DateTime.fromObject({
            year: 2026,
            month: 1,
            day: 5,
    ...
    it('should respect maintenance windows on work centers when reflowing work orders', () => {
      ...
      const wo1 = result.updatedWorkOrders.find(
        (wo) => wo.workOrderNumber === 'WO-1',
      )!;
      expect(
        DateTime.fromISO(wo1.startDate).equals(
          DateTime.fromObject({ year: 2030, month: 6, day: 3, hour: 8 }),
        ),
      ).toBe(true);
      expect(
        DateTime.fromISO(wo1.endDate).equals(
          DateTime.fromObject({ year: 2030, month: 6, day: 3, hour: 9 }),
        ),
      ).toBe(true);

      // Second WO must be placed after maintenance, i.e., starts at 10:00
      const wo2 = result.updatedWorkOrders.find(
        (wo) => wo.workOrderNumber === 'WO-2',
      )!;
      expect(
        DateTime.fromISO(wo2.startDate).equals(
          DateTime.fromObject({ year: 2030, month: 6, day: 3, hour: 10 }),
        ),
      ).toBe(true);
      expect(
        DateTime.fromISO(wo2.endDate).equals(
          DateTime.fromObject({ year: 2030, month: 6, day: 3, hour: 11 }),
        ),
      ).toBe(true);
    });
    ...
    it('should not move maintenance work orders, and should move other work orders around them', () => {
      const workOrders = [
        workOrderDocument('WO-MAINTENANCE', [], {
          workCenterId: 'WC-002',
          startDate:
            DateTime.fromObject({
              year: 2031,
      ...
      const maint = result.updatedWorkOrders.find(
        (wo) => wo.workOrderNumber === 'WO-MAINTENANCE',
      )!;
      expect(
        DateTime.fromISO(maint.startDate).equals(
          DateTime.fromObject({ year: 2031, month: 4, day: 14, hour: 10 }),
        ),
      ).toBe(true);
      expect(
        DateTime.fromISO(maint.endDate).equals(
          DateTime.fromObject({ year: 2031, month: 4, day: 14, hour: 11 }),
        ),
      ).toBe(true);
      expect(maint.isMaintenance).toBe(true);

      // WO-A scheduled after maintenance
      const woa = result.updatedWorkOrders.find(
        (wo) => wo.workOrderNumber === 'WO-A',
      )!;
      expect(
        DateTime.fromISO(woa.startDate).equals(
          DateTime.fromObject({ year: 2031, month: 4, day: 14, hour: 11 }),
        ),
      ).toBe(true);
      expect(
        DateTime.fromISO(woa.endDate).equals(
          DateTime.fromObject({ year: 2031, month: 4, day: 14, hour: 13 }),
        ),
      ).toBe(true);

      // WO-B scheduled after WO-A
      const wob = result.updatedWorkOrders.find(
        (wo) => wo.workOrderNumber === 'WO-B',
      )!;
      expect(
        DateTime.fromISO(wob.startDate).equals(
          DateTime.fromObject({ year: 2031, month: 4, day: 14, hour: 13 }),
        ),
      ).toBe(true);
      expect(
        DateTime.fromISO(wob.endDate).equals(
          DateTime.fromObject({ year: 2031, month: 4, day: 14, hour: 14 }),
        ),
      ).toBe(true);
    });
  });
